...
async function scheduleAlarm(intervalMinutes: number) {
...
chrome.runtime.onInstalled.addListener(async () => {
  const s = await getSettings();
  await scheduleAlarm(s.intervalMinutes);
...
chrome.runtime.onMessage.addListener((msg, _sender, respond) => {
  if (msg.action === 'reschedule') {
    scheduleAlarm(msg.intervalMinutes as number).then(() => respond({ ok: true }));
    return true;
  }
